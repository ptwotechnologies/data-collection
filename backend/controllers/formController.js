...
export const getRegistrations = async (req, res) => {
    try {
        ...
        const users = await Registration.find(query).sort({ [sortKey]: sortOrder === 'asc' ? 1 : -1 });
        res.json(users);
    } catch (err) {
...
export const updateRegistration = async (req, res) => {
    try {
      ...
      res.json({ message: 'Registration updated successfully', data: updatedUser });
    } catch (err) {
      res.status(400).json({ error: 'Failed to update registration', details: err.message });
    }
  };
...
export const importCSV = async (req, res) => {
    try {
      ...
      res.json({ message: 'CSV imported successfully', insertedCount: inserted.length });
    } catch (err) {
